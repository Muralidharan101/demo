...
/* Red button style for cells with Pending status */
.pending-cell {
  background-color: #FF5733 !important; /* Adjust the color as needed */
  color: white;
  font-weight: bold;
...
/* Green button style for cells with Approved status */
.approved-cell {
  background-color: #5cb85c !important; /* Adjust the color as needed */
  color: white;
  font-weight: bold;
  padding: 6px 12px; /* Adjust padding for button size */
  border-radius: 4px; /* For rounded corners */
  ...
  <script src="../../assets/vendor_components/datatable/datatables.min.js"></script>
    ...
    <script>
      $(document).ready(function(){
        ...
        $('#submit').click(function() {
            ...
            if (i_type == '') {
                ...
                if (poster) {
                    ...
                    uploadFile(null, brand_id, i_type, openAmount, closeAmount, c_type, startDate, endDate);
                }
            }
        });

        function uploadFile(file, id, inf_type, openAmount, closeAmount, c_type, startDate, endDate) {
            var fd = new FormData();
        ...
        columnDefs: [
          {
            targets: 6, // Replace 6 with the index of your camp_status column
            createdCell: function (td, cellData, rowData, row, col) {
              if (cellData === 'Pending') {
                $(td).addClass('pending-cell');
              } else if (cellData === 'Approved') {
                $(td).addClass('approved-cell');
              }
            }
          }
        ]
